...
- 更新完成后新增明确日志，便于区分“容器更新卡住”和“Telegram 消息编辑失败”。

## v0.7.12

- 修复点击“更新”后 Telegram 消息可能一直停留在“正在更新”的问题：更新任务不再继承 callback polling 请求上下文，避免 getUpdates 返回后后台更新被取消。
- 适配较慢的 `latest` 镜像更新流程，容器实际更新完成后能继续编辑为成功/失败结果。
